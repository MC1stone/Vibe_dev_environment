...
# NIR_MISTRAL Docker Startup Script
# This script helps start the Docker infrastructure

set -e

echo "🚀 Starting NIR_MISTRAL Docker Infrastructure"
echo "=============================================="
...
if [ ! -f "docker-compose.yml" ]; then
    echo "❌ Please run this script from the NIR_MISTRAL project root directory."
...
echo "📁 Current directory: $(pwd)"
echo "📁 Files in current directory: $(ls -1 | head -10 | tr '\n' ' ')"
...
if [ ! -f ".env" ]; then
    ...
    echo ""
    echo "💡 Edit the .env file to configure:"
...
if [ -z "${DJANGO_SECRET_KEY:-}" ] || [ "${DJANGO_SECRET_KEY:-}" = "your-production-secret-key-change-me" ]; then
    echo "⚠️  DJANGO_SECRET_KEY is not set or is default. Using a temporary key for development."
    DJANGO_SECRET_KEY="dev-secret-key-$(date +%s)"
    export DJANGO_SECRET_KEY
fi

if [ -z "${POSTGRES_PASSWORD:-}" ] || [ "${POSTGRES_PASSWORD:-}" = "nir_password_2026" ]; then
    echo "⚠️  POSTGRES_PASSWORD is not set or is default. Using a temporary password for development."
...
for ((i=1; i<=$MAX_RETRIES; i++)); do
    if docker-compose -f $COMPOSE_FILE exec postgresql pg_isready -U nir_user -d nir_mistral > /dev/null 2>&1; then
    ...
    if [ $i -eq $MAX_RETRIES ]; then
        ...
        echo "💡 Check the PostgreSQL logs with: docker-compose -f $COMPOSE_FILE logs postgresql"
        exit 1
    fi
done
...
if docker-compose -f $COMPOSE_FILE exec ollama ollama list | grep -q mistral; then
...
SUPERUSER_CHECK=$(docker-compose -f $COMPOSE_FILE exec django_app python django_project/manage.py shell -c "
...
if [ "$SUPERUSER_CHECK" = "superuser_exists" ]; then
    echo "✅ Superuser already exists."
elif [ "$SUPERUSER_CHECK" = "no_superuser" ]; then
    echo "👤 Creating superuser..."
    echo "Please enter the following details for the Django admin user:"
    docker-compose -f $COMPOSE_FILE exec django_app python django_project/manage.py createsuperuser
else
    echo "⚠️  Could not check superuser status. You may need to create one manually."
    echo "   Run: docker-compose -f $COMPOSE_FILE exec django_app python django_project/manage.py createsuperuser"
fi

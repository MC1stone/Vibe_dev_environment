...
HSWTStylingAgent is a specialized agent in the NIR Intelligence Platform responsible for HSWTStyling functionality.
...
HSWTStylingAgent requires the following dependencies:
...
```python
from agents.hswt_styling_agent import HSWTStylingAgent
...
HSWTStylingAgent handles the following error scenarios:
...
Run tests for HSWTStylingAgent:

```bash
# Unit tests
pytest tests/unit/test_hswt_styling_agent.py

# Integration tests
pytest tests/integration/test_hswt_styling_agent_integration.py

# End-to-end tests
pytest tests/e2e/test_hswt_styling_agent_e2e.py
```

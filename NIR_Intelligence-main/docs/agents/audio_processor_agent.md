...
AudioProcessorAgent is a specialized agent in the NIR Intelligence Platform responsible for AudioProcessor functionality.
...
AudioProcessorAgent requires the following dependencies:
...
```python
from agents.audio_processor_agent import AudioProcessorAgent
...
AudioProcessorAgent handles the following error scenarios:
...
Run tests for AudioProcessorAgent:

```bash
# Unit tests
pytest tests/unit/test_audio_processor_agent.py

# Integration tests
pytest tests/integration/test_audio_processor_agent_integration.py

# End-to-end tests
pytest tests/e2e/test_audio_processor_agent_e2e.py
```
